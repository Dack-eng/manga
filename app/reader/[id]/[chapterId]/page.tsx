...
export default function ReaderPage({ params: paramsPromise }: { params: Promise<{ id: string, chapterId: string }> }) {
  ...
  // Mocking manga pages (in a real app, these would come from the chapter data)
  const pages = [
    manga.cover, // Using cover as temporary pages for demo
    manga.banner,
    manga.cover,
  ];
  ...
  return (
    <div className="min-h-screen bg-black text-white selection:bg-primary/30">
      ...
      >
        ...
        <div className="mt-12 px-4 space-y-8 text-center">
          <div className="flex items-center justify-center gap-4">
            {prevChapter && (
              <Link 
                href={`/reader/${manga.id}/${prevChapter.id}`} 
            ...
            {nextChapter && (
              <Link 
                href={`/reader/${manga.id}/${nextChapter.id}`} 
          ...
          <Link href={`/manga/${manga.id}`} className="inline-block text-sm font-semibold text-zinc-400 hover:text-white transition-colors">
            Мэдээлэл рүү буцах
          </Link>
        </div>
      </main>
      ...
      {/* Click zones for simple nav */}
      <div 
        className="fixed inset-y-0 left-0 w-24 z-10 cursor-pointer" 
        onClick={(e) => {
          e.stopPropagation();
      ...
      />
      <div 
        className="fixed inset-y-0 right-0 w-24 z-10 cursor-pointer" 
        onClick={(e) => {
          e.stopPropagation();
          setShowControls(!showControls);
        }}
